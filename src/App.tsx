...
const AutomationDemo: React.FC<{ language: Language }> = ({ language }) => {
    ...
    return (
        <div className="h-full flex flex-col bg-canvas-subtle p-5 relative overflow-hidden select-none">
            <div className="relative z-10 mb-6">
                ...
                <div className="grid grid-cols-4 w-full">{steps.map((s, i) => (<div key={i} className={`flex flex-col items-center gap-2 transition-all duration-300 ${i <= step ? 'opacity-100' : 'opacity-40'}`}><div className={`w-5 h-5 rounded-full flex items-center justify-center text-[9px] font-bold border transition-all duration-500 bg-black ${i < step ? 'bg-primary-DEFAULT border-primary-DEFAULT text-black' : i === step ? 'border-primary-DEFAULT text-primary-DEFAULT shadow-[0_0_10px_cyan]' : 'border-slate-600 text-slate-500'}`}>{i < step ? '✓' : i + 1}</div><span className={`text-[8px] uppercase font-bold tracking-wider text-center w-full truncate ${i === step ? 'text-primary-DEFAULT' : 'text-slate-400'}`}>{s}</span></div>))}</div>
            </div>
            <div className="flex-1 flex flex-col gap-4 min-h-0">
                <div className="h-24 relative border border-white/10 bg-black/30 rounded-lg flex items-center justify-center overflow-hidden shrink-0"><svg className={`w-10 h-10 transition-all duration-500 ${status === 'done' ? 'text-status-success scale-110' : 'text-white'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>{status === 'processing' && <div className="absolute inset-0 z-10 animate-[scanline_2s_linear_infinite]"><div className="h-0.5 w-full bg-primary-DEFAULT shadow-[0_0_15px_cyan]"></div></div>}</div>
                <div ref={scrollRef} className="flex-1 bg-[#050507] border border-white/10 rounded-lg p-3 overflow-y-auto custom-scrollbar font-mono text-[9px] relative shadow-inner-light">{logs.map((l, i) => <div key={i} className="mb-1.5 flex gap-2 animate-fade-in-up"><span className={l.type === 'info' ? 'text-blue-400' : l.type === 'success' ? 'text-emerald-400' : 'text-amber-400'}>[{l.type.toUpperCase()}]</span><span className="text-slate-300">{l.text}</span></div>)}</div>
            </div>
            {(status === 'idle' || status === 'done') && <button onClick={startProcess} className="mt-4 w-full py-3 bg-white hover:bg-slate-200 text-black font-bold uppercase text-xs tracking-widest rounded shadow-[0_0_20px_rgba(255,255,255,0.2)] transition-all active:scale-95 z-20">{logTexts.btn}</button>}
        </div>
    );
};
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 w-full flex-grow flex flex-col justify-center">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-8 items-center">
                    ...
                    <div className="order-2 lg:order-1 flex flex-col items-center lg:items-start text-center lg:text-left relative z-40">
                        ...
                        <p className="text-base md:text-xl text-slate-400 max-w-xl leading-relaxed mb-8 md:mb-10 animate-fade-in-up [animation-delay:400ms]">
                        ...
                        <div className="flex flex-col sm:flex-row items-center gap-4 w-full sm:w-auto animate-fade-in-up [animation-delay:600ms]">
                            <NeonButton onClick={() => setCurrentView('ai-hub')} icon={<svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>} fullWidth={true}>{TEXTS[language].cta}</NeonButton>
                            <NeonButton variant="secondary" onClick={() => setCurrentView('services')} fullWidth={true}>{TEXTS[language].learnMore}</NeonButton>
                        </div>
                    </div>
