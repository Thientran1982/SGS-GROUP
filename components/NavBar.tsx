...
const NavBar: React.FC<NavBarProps> = ({ 
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Smart Hide/Show (Dynamic Island Behavior)
  ...
  return (
    <>
      ...
      `}>
        {/* Main HUD Container */}
        <div className={`pointer-events-auto relative rounded-full flex items-center justify-between gap-1.5 md:gap-8 shadow-[0_8px_32px_-8px_rgba(0,0,0,0.1)] dark:shadow-[0_0_40px_rgba(6,182,212,0.1)] border transition-all duration-500 hover:shadow-[0_15px_40px_-10px_rgba(59,130,246,0.2)] w-full max-w-[98%] md:max-w-fit backdrop-blur-xl overflow-hidden group/nav-container
            ${scrolled 
                ? 'bg-white/80 dark:bg-[#050505]/80 border-slate-200/80 dark:border-white/10 p-1.5 pl-3 md:pl-4 pr-1.5 backdrop-saturate-150' 
                : 'bg-white/60 dark:bg-[#050505]/50 border-white/40 dark:border-white/5 p-1.5 md:p-2.5 md:pl-6 md:pr-2.5'
            }
        `}>
          ...
          <div className="hidden md:flex items-center bg-slate-100/50 dark:bg-white/5 rounded-full p-1 border border-white/40 dark:border-white/5 backdrop-blur-md">
            {NAV_ITEMS.map((item) => (
              <MagneticButton
                key={item.id}
              ...
              >
                {currentView === item.id && (
                  <div className="absolute inset-0 bg-slate-900 dark:bg-cyan-600 rounded-full">
                      <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/20 to-transparent -translate-x-full animate-[scan_3s_infinite]"></div>
                  </div>
                )}
                {currentView !== item.id && (
          ...
          <div className="flex items-center gap-1.5 md:gap-3 pl-2 md:pl-6 md:border-l border-slate-200/80 dark:border-white/10 h-8">
              ...
              <button 
                  ...
                  className="relative w-10 md:w-12 h-6 md:h-7 rounded-full border border-slate-200 dark:border-white/20 bg-slate-100 dark:bg-black/40 overflow-hidden group transition-all hover:border-blue-400 dark:hover:border-cyan-400"
                  title="Toggle Language"
              >
                   <div className={`absolute top-0.5 bottom-0.5 w-[50%] bg-white dark:bg-cyan-600 rounded-full shadow-sm transition-all duration-500 cubic-bezier(0.175, 0.885, 0.32, 1.275) ${language === 'en' ? 'left-0.5' : 'left-[46%]'}`}></div>
              ...
              <button
                  ...
                  className={`relative w-8 h-8 rounded-full flex items-center justify-center border transition-all duration-500 overflow-hidden group flex-shrink-0
                    ${theme === 'light' 
                        ? 'bg-amber-100 border-amber-300 shadow-[0_0_15px_rgba(251,191,36,0.4)]' 
                        : 'bg-slate-900 border-cyan-900 shadow-[0_0_15px_rgba(6,182,212,0.2)]'
                    }`}
                  title="Switch Theme"
              >
                  <div className={`absolute transition-all duration-500 ease-in-out ${theme === 'light' ? 'opacity-100 rotate-0 scale-100' : 'opacity-0 rotate-90 scale-50'}`}>
                      <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4 text-amber-500" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2.25a.75.75 0 01.75.75v2.25a.75.75 0 01-1.5 0V3a.75.75 0 01.75-.75zM7.5 12a4.5 4.5 0 119 0 4.5 4.5 0 01-9 0zM18.894 6.166a.75.75 0 00-1.06-1.06l-1.591 1.59a.75.75 0 101.06 1.061l1.591-1.59zM21.75 12a.75.75 0 01-.75.75h-2.25a.75.75 0 010-1.5H21a.75.75 0 01.75.75zM17.834 18.894a.75.75 0 001.06-1.06l-1.59-1.591a.75.75 0 10-1.061 1.06l1.59 1.591zM12 18a.75.75 0 01.75.75V21a.75.75 0 01-1.5 0v-2.25A.75.75 0 0112 18zM7.758 17.303a.75.75 0 00-1.061-1.06l-1.591 1.59a.75.75 0 001.06 1.061l1.591-1.59zM6 12a.75.75 0 01-.75.75H3a.75.75 0 010-1.5h2.25A.75.75 0 016 12zM6.697 7.757a.75.75 0 001.06-1.06l-1.59-1.591a.75.75 0 00-1.061 1.06l1.59 1.591z" /></svg>
                  </div>
              ...
              {/* Mobile Menu Trigger */}
               <button 
                  ...
                  className={`md:hidden w-8 h-8 rounded-full flex items-center justify-center shadow-lg active:scale-95 transition-all duration-200 border flex-shrink-0 touch-manipulation
                    ${isMobileMenuOpen 
                        ? 'bg-red-500 text-white border-red-400 rotate-90' 
                        : 'bg-slate-900 dark:bg-white text-white dark:text-black border-transparent hover:border-cyan-400'
                    }`}
              >
                  {isMobileMenuOpen ? (
      ...
      {/* Mobile Menu Overlay */}
      <div className={`fixed inset-0 z-40 bg-[#f8fafc]/95 dark:bg-[#020408]/95 backdrop-blur-3xl transition-all duration-300 ease-[cubic-bezier(0.32,0.72,0,1)] flex flex-col ${isMobileMenuOpen ? 'opacity-100 translate-y-0 pointer-events-auto' : 'opacity-0 -translate-y-full pointer-events-none'}`}>
         ...
         <div className="absolute inset-x-0 h-1 bg-cyan-500/50 blur opacity-30 animate-scanline pointer-events-none"></div>
            ...
            <div className="w-full space-y-3">
                {NAV_ITEMS.map((item, idx) => (
                    <button
                        ...
                        className={`w-full py-4 px-6 text-xl font-display font-bold text-left transition-all duration-500 transform border border-slate-200 dark:border-white/10 rounded-xl relative overflow-hidden group hover:scale-[1.02] active:scale-95 touch-manipulation
                            ${currentView === item.id 
                                ? 'bg-white dark:bg-cyan-900/20 border-blue-500 dark:border-cyan-500 shadow-[0_0_20px_rgba(6,182,212,0.15)] translate-x-2' 
                    ...
                    >
                        {currentView === item.id && <div className="absolute left-0 top-0 bottom-0 w-1.5 bg-blue-500 dark:bg-cyan-500"></div>}
